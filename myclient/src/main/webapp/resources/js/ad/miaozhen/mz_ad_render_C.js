var mzadxC = (function(mzadxC, win, doc) {
    ...
    _onload = win.onload,
    MZ_RSQ = "__mz_ad_rsq",
    render_pre_data_queue = "__mz_render_data_queue";
    ...
    function render_material(material, node) {
        ...
        if (imp.constructor == Object) {
            ...
            for (var sec in imp) {
                var sximp = imp[sec];
                if (sximp.constructor == Array) { (function(sximp, sec) {
        ...
        if (material.src.toLowerCase().indexOf("<iframe") == 0) {
            ...
            if (style) {
                var data = {};
                data.material = material;
                data.contentHTML = '<iframe width="' + material.adw + '" height="' + material.adh + '" frameborder="0" marginwidth="0" marginheight="0" allowTransparency="true" scrolling="no" src="http://js.miaozhen.com/mz_iframed_js.html#' + encodeURIComponent(material.src.split("").reverse().join("")) + '"></iframe>';
                pushDataToRenderStyleQueue(data)
            } else {
                node.innerHTML = '<iframe width="' + material.adw + '" height="' + material.adh + '" frameborder="0" marginwidth="0" marginheight="0" allowTransparency="true" scrolling="no" src="http://js.miaozhen.com/mz_iframed_js.html#' + encodeURIComponent(material.src.split("").reverse().join("")) + '"></iframe>'
            }
        }
    }
